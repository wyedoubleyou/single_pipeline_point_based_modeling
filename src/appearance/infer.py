...
def infer_appearance(data_dict, ndesc_stack, renderer, renderer_flood, generator, device):
    ...
    # print("data_dict['ndesc']", data_dict['ndesc'] )
    # print("len of data_dict['ndesc']", len(data_dict['ndesc'][0][0]) )
    # itw 16 
    
    # get rasterization info
    data_dict['smpl_depth'] = get_smpl_depth(renderer_flood, data_dict).permute(0, 3, 1, 2)
    # print("data_dict['smpl_depth']", data_dict['smpl_depth'])
    # print("len of data_dict['smpl_depth']", len(data_dict['smpl_depth'][0][0][0]))
    # itw 256
    
    data_dict['cloth_depth'], data_dict['dmin'] = renderer_flood.render_cloth_depth(data_dict) #20/7 error here
...
def do_vton_overlays(data_dict):
    ...
    real_rgb_nos_np = data_dict['real_rgb_nos_np'] #here already cropped short 
    # plt.imshow(real_rgb_nos_np)

    #yw to note: modify 
    real_rgb_nos_np=transform.resize(real_rgb_nos_np,(256,256,3))
    raster_mask_np = data_dict['raster_mask_np']
    raster_features_np = data_dict['raster_features_np']
...
#yw to note : error here got cropped 
def infer_pid(style_pid, data_dict, target_dict, outfit_code, ndesc_stack, converter,
    ...
    # print("data_dict after infer geometry", data_dict)
    #20/7  error here 
    ...
    # smpl_rast1=smpl_rast[0][0].detach().cpu().numpy()
    # plt.imshow(smpl_rast1)
    # plt.savefig(f'out/appearance/smpl_mask.png')
    out_dict['smpl_rast'] = smpl_rast
    
    # print(out_dict.keys())
    # dict_keys(['real_rgb', 'real_segm', 'real_rgb_nos', 'smpl_pose', 'smpl_verts_cam', 'smpl_verts_world', 'K', 'seq', 'rgb_path', 'source_pcd', 'betas', 'smpl_transl', 'smpl_glor', 'index', 'fid', 'cam', 'cloth_pcd', 'glo_vec', 'pids_order', 'ndesc', 'smpl_depth', 'cloth_depth', 'dmin', 'raster_mask', 'raster_features', 'visibilty_mask', 'smpl_mask', 'smpl_rast', 'flood_raster_mask', 'flood_raster_features', 'flood_visibilty_mask', 'flood_smpl_mask', 'flood_smpl_rast', 'flood_cloth_mask', 'fake_rgb', 'fake_segm', 'fake_rgb_bbg'])
